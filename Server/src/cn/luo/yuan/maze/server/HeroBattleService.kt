 ...
 */
class HeroBattleService(private val tableCache: Map<String, HeroTable>) : Runnable, RunningServiceInterface {
    ...
    private fun range(){
        ...
        for(id in sortedByDescending){
            tableCache[id]!!.getRecord(id).range = sortedByDescending.indexOf(id)
        }
    }
    ...
    private fun filterMatch(id:String,level:Long):String?{
        val items = ArrayList<String>(tableCache.keys.filter {
            val table = tableCache[it] as HeroTable
            val maze = table.getMaze(it)
            it!=id && Math.abs(maze.maxLevel - level) < 100
        })
