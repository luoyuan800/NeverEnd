...
public class PermissionHelper {
    ...
    private PermissionModel[] mPermissionModels = new PermissionModel[]{
            new PermissionModel("电话", Manifest.permission.READ_PHONE_STATE, "我们需要读取手机标识的权限来标识您的身份", READ_PHONE_STATE_CODE),
    ...
    public void applyPermissions() {
        try {
            for (final PermissionModel model : mPermissionModels) {
                if (PackageManager.PERMISSION_GRANTED != mActivity.checkSelfPermission(model.permission)) {
                    mActivity.requestPermissions(new String[]{model.permission}, model.requestCode);
                    return;
                }
            }
        ...
        switch (requestCode) {
            ...
            case WRITE_EXTERNAL_STORAGE_CODE:
                ...
                if (PackageManager.PERMISSION_GRANTED != grantResults[0]) {
                    ...
                    // 方便用户理解我们为什么需要这个权限
                    if (mActivity.shouldShowRequestPermissionRationale(permissions[0])) {
