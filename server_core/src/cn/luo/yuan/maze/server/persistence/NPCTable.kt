...
constructor(root: File) : HeroTable(root) {
    val heroDb = FileObjectTable<Hero>(Hero::class.java, root)
    ...
    private fun randomNPC(level: Long): Hero {
        ...
        hero.hpGrow = 10;
        if(name.contains("流氓")){
            hero.atkGrow *= random.nextInt(20) + 1
            hero.hpGrow *= random.nextInt(30) + 1
        }
        if(name.contains("贱客")){
            hero.atkGrow *= random.nextInt(10) + 1
            hero.defGrow *= random.nextInt(15) + 1
        }
        hero.agi = 100 * level
        hero.def += hero.defGrow * hero.agi
        hero.str = 100 * level
